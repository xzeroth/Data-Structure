...
class AVLTree {
 ...
 private:
  // Declare a root node
  AVLNode* newNode(string x);
  AVLNode* testInsert(const string& x, AVLNode*& node);
  AVLNode* root;
  int nodeCounter;
  void balanceIt(AVLNode*& n);
  void insert(const string& x, AVLNode*& node);
  bool finda(const string& x, AVLNode*& node, vector<string>& path);
